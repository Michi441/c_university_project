            ...
            for (int v = 0; v < sl2->count; v++)
            {
                if (sl2->stadt[v]->stadtId == selectedRoads->street[u]->stadtEnd)
                {
                    ...
                    if (s->gebietId < (sl2->stadt[v]->gebietId * 2))
                    {
                        ...
                        WriteAfterMathData(sl2, s, "stadtneu.dat");
                    }
                    else
                    {
...
void WriteAfterMathData(StadtList *sl3, Stadt *s, char *fileName)
{
    ...
    {
        if (sl3->stadt[i]->stadtId == s->stadtId)
        {
            fprintf(file, "%s %d %s\n", sl3->stadt[i]->name, sl3->stadt[i]->gebietId, "[Eliminated]");
        }
        else
        {
            fprintf(file, "%s %d \n", sl3->stadt[i]->name, sl3->stadt[i]->gebietId);
        }
    }
