...
{
    struct CityStreetArray arr[sl2->count];
        ...
        if (streetList->street[i]->stadtStart == s->stadtId)
        {
            ...
            for (int g = 0; g < sl2->count; g++)
            {
                ...
                if (sl2->stadt[g]->stadtId == streetList->street[i]->stadtEnd)
                {
                    ...
                    arr[g].einwohner = sl2->stadt[g]->einwohner;
                    arr[g].gebietId = sl2->stadt[g]->gebietId;
                    arr[g].stadtId = sl2->stadt[g]->stadtId;
                    arr[g].stadtStart = streetList->street[i]->stadtStart;
                    arr[g].stadtEnd = streetList->street[i]->stadtEnd;
                    arr[g].distance = streetList->street[i]->distance;
                }
            }
    ...
    {
        printf("%s %d", "POPULATION", arr[c].distance);
        printf("%s \n", sl3->stadt[c]->name);
