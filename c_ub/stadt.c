...
void TheDestroyer(StadtList *sl, RegionList *rl, int id, StreetList *streetList)
{
    ...
    if (s == NULL)
    {
        ...
        printf("%s \n \n", "No city was destroyed on that day!");
        ...
        if (selectedRoads->street[u]->stadtStart == s->stadtId)
        {
            ...
            for (int v = 0; v < sl2->count; v++)
            {
                if (sl2->stadt[v]->stadtId == selectedRoads->street[u]->stadtEnd)
                {
                    ...
                    // CITY TO GET POPULATION

                    if (s->gebietId < (sl2->stadt[v]->gebietId * 2))
                    {
                        sl2->stadt[v]->gebietId = sl2->stadt[v]->gebietId + s->gebietId;
                        s->gebietId = 0;

                        printf("%s %d \n", "yes, its smaller", sl2->stadt[v]->gebietId);
                        printf("%s %d \n", "yes, its smaller", s->gebietId);
                        WriteAfterMathData(sl2, s, sl2->stadt[v], "stadtneu.dat");
                    }
                    else
                    {
                        s->gebietId = s->gebietId - (popOtherCity * 2);
                        sl2->stadt[v]->gebietId = sl2->stadt[v]->gebietId * 2;
...
//             fprintf(file, "Stadt | Population\n \n");

//             fprintf(file, "%s %d \n", sl3->stadt[c]->name, newPop);
//             fprintf(file, "%s %d \n", s->name, oldPop);
...
// // Sort the array for the final population swap

void WriteAfterMathData(StadtList *sl3, Stadt *s, Stadt *s2, char *fileName)
{
    printf("%s", "new population size is:");
    int ii = 0;
    FILE *file = fopen("stadtneu.dat", "w");
    fprintf(file, "ID | Stadt | Population\n \n");
    for (int i = 0; i < sl3->count; i++)
    {
        if (sl3->stadt[i]->stadtId == s2->stadtId)
        {
            fprintf(file, "%d %s %d %s\n", sl3->stadt[i]->stadtId, sl3->stadt[i]->name, sl3->stadt[i]->gebietId, "[Received all refuges]");
        }
        else if (sl3->stadt[i]->stadtId == s->stadtId)
        {
            fprintf(file, "%d %s %d %s\n", sl3->stadt[i]->stadtId, sl3->stadt[i]->name, sl3->stadt[i]->gebietId, "[Eliminated]");
        }
