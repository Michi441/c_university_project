...
void GetAllDistancesFromCity(StadtList *sl2, Stadt *s, StreetList *streetList)
{
    ...
    for (int u = 0; u < selectedRoads->count; u++)
    {
        ...
        {
            for (int v = 0; v < sl3->count; v++)
            {
                if (sl3->stadt[v]->stadtId == selectedRoads->street[u]->stadtEnd)
                {
                    printf("%s %s %d \n\n", sl3->stadt[v]->name, "population:", sl3->stadt[v]->gebietId);
                }
            }
        }
        ...
        if (selectedRoads->street[u]->stadtEnd == s->stadtId)
        {
            for (int v = 0; v < sl3->count; v++)
            {
                ...
                if (sl3->stadt[v]->stadtId == selectedRoads->street[u]->stadtStart)
                {
                    printf("%s %s %d %d \n\n", sl3->stadt[v]->name, "population:", sl3->stadt[v]->gebietId, popOtherCity);
                }
            }
            // Stadt *rt = findStadt(sl3, selectedRoads->street[u]->stadtStart);
            // popOtherCity = rr->gebietId;
        }
        printf("%s %d \n", "Distance: ", selectedRoads->street[u]->distance);
    }
}
